...
describe('myERC20', function () {
  ...
  describe('Transfer operations', () => {
    ...
    it("Should decrease amount of tokens at owner's acc by transfer amount", async () => {
      ...
      const ownerBalanceBeforeTransfer: bigint = await myERC20.balanceOf(
        owner.address,
      );

      await myERC20.connect(owner).transfer(user1.address, transferAmount);

      const ownerBalanceAfterTransfer: bigint = await myERC20.balanceOf(
        owner.address,
      );
    ...
    it('Should not allow transfers exceeding the balance', async () => {
      const initialBalanceUser1: bigint = await myERC20.balanceOf(
        user1.address,
      );
    ...
    it('Should set allowance correctly', async () => {
      ...
      const allowance: bigint = await myERC20.allowance(
        owner.address,
        user1.address,
      );
    ...
    it('Should not allow transfer more than approved amount', async () => {
      ...
      await expect(
        myERC20
          .connect(user1)
          .transferFrom(owner.address, user1.address, amountToTransfer),
      ).to.be.reverted;
    });
  ...
  describe('Token operations', () => {
    it('Should allow user to buy tokens', async () => {
      ...
      const expectedAmount: bigint =
        ethers.parseEther(ethersSent.toString()) / votePrice;
    ...
    it('Should deny user to buy tokens if user address is 0', async () => {
      await expect(myERC20.testMintToZero()).to.be.revertedWith(
        'Recipient can not be 0!',
      );
    });
  ...
  describe('Voting', () => {
    ...
    it('Should deny user with amount of 0.05% to 0.1% from total supply of tokens to start vote', async () => {
      ...
      const lessThanThreshold: bigint =
        (initialMint * BigInt(6)) / BigInt(10000);

      await myERC20.connect(owner).transfer(user1.address, lessThanThreshold);

      await expect(
        myERC20.connect(user1).vote(testVotePrice),
      ).to.be.revertedWith('You can not start voting!');
    });

    it('Should deny user with amount less than 0.05% from total supply of tokens to vote', async () => {
      const testVotePrice: bigint = BigInt(100);
      const lessThanVoteQuote: bigint =
        (initialMint * BigInt(4)) / BigInt(10000);
      ...
      await expect(
        myERC20.connect(user1).vote(testVotePrice),
      ).to.be.revertedWith('You can not vote!');
    });
    ...
    it('Should protect from the double voting by the same user', async () => {
      ...
      await expect(
        myERC20.connect(owner).vote(anotherVotePrice),
    ...
    it('Should get power of price in current voting', async () => {
      ...
      const user1PricePower: bigint = await myERC20._getPowerOfVotingPrice(
        votePrices[0],
      );
      ...
      await myERC20.connect(owner).vote(votePrices[1]);
      const ownerPricePower: bigint = await myERC20._getPowerOfVotingPrice(
        votePrices[1],
      );
    ...
    it('Should deny end voting before it period expired', async () => {
      ...
      await expect(myERC20.endVote()).to.be.revertedWith(
    ...
    it("Should deny to sell tokens if person voted and voting hasn't ended yet.", async () => {
      ...
      await expect(
        myERC20.connect(owner).sell(tokensToSell),
      ).to.be.revertedWith(
        'You are not allowed perform this during the voting!',
      );
    });
  ...
  describe('Fee operations', () => {
    it('Should allow only owner to set fee percentage', async () => {
      await expect(myERC20.connect(user1).setBuySellFeePercentage(10)).to.be
        .reverted;
    });

    it('Should allow to set only proper fee percentage', async () => {
      await expect(
        myERC20.connect(owner).setBuySellFeePercentage(110),
      ).to.be.revertedWith('Percentage must be less than or equal to 100');
    });

    it('Should collect fees from buying operations', async function () {
      const votePrice: bigint = BigInt(100);
      const ethersSent: bigint = BigInt(1);
      const tokToBuy: bigint =
        ethers.parseEther(ethersSent.toString()) / BigInt(100);
      const expectedReceivedToks: bigint =
        tokToBuy - (tokToBuy * BigInt(10)) / BigInt(100);
    ...
    it('Should collect fees from selling operations', async function () {
      ...
      const sellFee: bigint =
        (tokToSell * votePrice * BigInt(10)) / BigInt(100);
      ...
      const totalCost: bigint =
        gasPrice && gasUsed ? gasPrice * gasUsed : BigInt(0);
      const finalBalance: bigint = await ethers.provider.getBalance(
        owner.address,
      );

      expect(finalBalance).to.equal(
        initialBalance - totalCost + expectedReceivedEth,
      );
    });
    ...
    it('Should burn collected fees', async function () {
      ...
      const finalTokenBalance: bigint = await myERC20.balanceOf(
  ...
  describe('Events', () => {
    ...
    it('Should emit EndVote event', async function () {
      ...
      await expect(voteEnded)
        .to.emit(myERC20, 'VotingEnded')
        .withArgs(blockTime, votePrice);
    });
  });
});
